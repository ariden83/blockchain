...
func (e *Explorer) loadRoutes() {
	...
	e.router.HandleFunc("/terms/service", e.termsOfServicePage).Methods(http.MethodGet)
	e.router.HandleFunc("/sitemap", e.sitemapPage).Methods(http.MethodGet)

	e.router.HandleFunc("/inscriptiona", e.inscriptionAPI).Methods(http.MethodPost)
}
